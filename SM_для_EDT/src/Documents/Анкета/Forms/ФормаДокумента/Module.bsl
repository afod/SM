...
Процедура ПриОткрытии(Отказ)
	...
	// ++ rarus zalikh 2021-05-06 [27313]
	ОбновлениеЗаголовковКнопокЗамечаний();
	// -- rarus zalikh 2021-05-06 [27313]
КонецПроцедуры
...
// СтандартныеПодсистемы.ПодключаемыеКоманды
&НаКлиенте
Процедура Подключаемый_ВыполнитьКоманду(Команда) Экспорт // ++ rarus makole 2021-02-09 
...
&НаКлиенте
Процедура Подключаемый_ОбновитьКоманды() Экспорт // ++ rarus makole 2021-02-09
	ПодключаемыеКомандыКлиентСервер.ОбновитьКоманды(ЭтотОбъект, Объект);
КонецПроцедуры
// Конец СтандартныеПодсистемы.ПодключаемыеКоманды

&НаКлиенте
Процедура ОткрытьСписокЗамечаний(Команда) // ++ rarus zalikh 2021-04-27
		
	тИмяКоманды = ЭтаФорма.ТекущийЭлемент.Имя;
	тИмяКоманды = Сред(тИмяКоманды, 8); // берем именно данные по уид
	тИмяКоманды = СтрЗаменить(тИмяКоманды,"_","-");
	тУИД = Новый УникальныйИдентификатор(тИмяКоманды);
		
	СтруктураОтбора = Новый Структура;
	СтруктураОтбора.Вставить("КлючСтроки", тУИД);
	МассивСтрок = ТаблицаВопросовРаздела.НайтиСтроки(СтруктураОтбора);
	
	//Найти коментарий строки
	//// ++ ilshil 12.07.2021	
	ИмяВопроса =   АнкетированиеКлиентСервер.ПолучитьИмяВопроса(МассивСтрок[0].КлючСтроки);
	ОткрытыйОтвет = ЭтотОбъект[ИмяВопроса + "_Комментарий"];
	//// ++ ilshil 12.07.2021
	
	ПараметрыФормы = Новый Структура("ОтборПоМероприятиюСУБ, ОтборПоВопросуДляАнкетирования, ОткрытыйОтвет",
	Объект.рарусМероприятиеСУБ, МассивСтрок[0].ЭлементарныйВопрос, ОткрытыйОтвет);
	
	Форма = ПолучитьФорму("Справочник.vftНесоответствия.Форма.ФормаСписка", ПараметрыФормы);
	
	Если Форма.Открыта() Тогда
		Форма.Закрыть();
	Конецесли;

	ОткрытьФорму("Справочник.vftНесоответствия.Форма.ФормаСписка", ПараметрыФормы);
	
КонецПроцедуры // -- rarus zalikh 2021-04-27

&НаКлиенте
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник) // ++ rarus zalikh 2021-04-27
...
&НаКлиенте
Процедура ОбновлениеЗаголовковКнопокЗамечаний() // ++ rarus zalikh 2021-05-06
	Для каждого тЭлемент Из Элементы Цикл
		Если СтрНачинаетсяС(тЭлемент.Имя,"Кнопка_") Тогда
			тИмяКоманды = Сред(тЭлемент.Имя, 8); // берем именно данные по уид
			тИмяКоманды = СтрЗаменить(тИмяКоманды,"_","-");
			тУИД = Новый УникальныйИдентификатор(тИмяКоманды);
			...
			СтруктураОтбора.Вставить("КлючСтроки", тУИД);
			МассивСтрок = ТаблицаВопросовРаздела.НайтиСтроки(СтруктураОтбора);
			КолЗамечанийПоТекВопросу = НайтиЗамечанияПоТекВопросу(Объект.рарусМероприятиеСУБ, МассивСтрок[0].ЭлементарныйВопрос); 
			Если КолЗамечанийПоТекВопросу <> 0 Тогда
				тЭлемент.Заголовок = "Замечания (" + Строка(КолЗамечанийПоТекВопросу) + ")";
			КонецЕсли;
		КонецЕсли;
	КонецЦикла; 
КонецПроцедуры // -- rarus zalikh 2021-05-06
	
&НаСервереБезКонтекста
Функция НайтиЗамечанияПоТекВопросу(тМероприятиеСУБ, тВопрос) // ++ rarus zalikh 2021-05-06
	Возврат рарусСУБСервер.НайтиКоличествоЗамечанийПоМероприятиюСУБИВопросу(тМероприятиеСУБ, тВопрос);
КонецФункции // -- rarus zalikh 2021-05-06
