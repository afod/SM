...
Процедура ПередЗаписью(Отказ)
	...
	// ++ rarus makole 2021-02-09 [27081]
	// При загрузке пользователя из главного узла, обработчики должны выполняться
	//Если ОбменДанными.Загрузка Тогда
	Если ОбменДанными.Загрузка 
		И Не ОбменДанными.Отправитель = ОбменДаннымиСервер.ГлавныйУзел() Тогда
	// -- rarus makole 2021-02-09 [27081]	
		Возврат;
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	// ++ rarus makole 2020-12-15 [Задача № 25921]
	// ПРОТОКОЛ ДЕМОНСТРАЦИИ № Р-62 от 14.12.2020
	// В системе (как на судне так и на берегу) должна быть возможность в карточке пользователя 
	// задать привязку к ФИО (строкой или выбором из справочника Физ. лица) 
	// с сохранением истории изменения этой привязки (период - день)
	Если Не Отказ Тогда
		РегистрыСведений.рарусСоответствиеПользователяФизическомуЛицу.СохранитьФизическоеЛицоПользователя(Ссылка, ФизическоеЛицо, рарусЗаведование);
	КонецЕсли;
	// -- rarus makole 2020-12-15 [Задача № 25921]
