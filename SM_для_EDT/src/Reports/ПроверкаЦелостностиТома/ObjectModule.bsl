...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// ++ rarus PleA 17.05.2021 [29125 ]
	ТомПараметр = Новый ПараметрКомпоновкиДанных("Том");
	Том = КомпоновщикНастроек.ФиксированныеНастройки.ПараметрыДанных.НайтиЗначениеПараметра(ТомПараметр);
	Если Том <> Неопределено тогда
		Если Том.Использование тогда
			КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра(ТомПараметр, Том.Значение);
			Том.Использование = Ложь;
		КонецЕсли;
	КонецЕсли;	
	// -- rarus PleA
