...
//++ rarus zakikh
		Объект.СтепеньРиска = Справочники.рарусСтепениРиска.Average;
//-- rarus zakikh
	КонецЕсли;
...
// ++ rarus zalikh
		Форма.Элементы.ТаблицаВариантыОтветовЗаполнитьВариантыПоУмолчанию.Доступность = Ложь;
// -- rarus zalikh
	Иначе
...
// ++ rarus zalikh
		Форма.Элементы.ТаблицаВариантыОтветовЗаполнитьВариантыПоУмолчанию.Доступность = Истина;
// -- rarus zalikh
	КонецЕсли; 
...
Процедура РедактированиеФормулировкиПриЗакрытии(ТекстВозврата, ДополнительныеПараметры) Экспорт
	...
	КонецЕсли;
// ++ rarus zalikh
	Если Объект.Формулировка <> ""  Тогда
...
&НаКлиенте
Процедура КомментарийКВопросуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("РедактированиеКомментарияКВопросуПриЗакрытии", ЭтотОбъект);
	ОбщегоНазначенияКлиент.ПоказатьФормуРедактированияМногострочногоТекста(ОповещениеОЗакрытии, Элемент.ТекстРедактирования, НСтр("ru = 'Формулировка';
																																	|en = 'Wording'"));
КонецПроцедуры

&НаКлиенте
Процедура РедактированиеКомментарияКВопросуПриЗакрытии(ТекстВозврата, ДополнительныеПараметры) Экспорт
	Если Объект.КомментарийКВопросу <> ТекстВозврата Тогда
		Объект.КомментарийКВопросу = ТекстВозврата;
		Модифицированность = Истина;
	КонецЕсли;
КонецПроцедуры
...
// ++ rarus zalikh
&НаКлиенте
Процедура рарусЗаполнитьВариантыПоУмолчанию(Команда)
	ТекущиеДанные = Элементы.ТаблицаВариантыОтветов.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("ЗаполнениеВариантовОтветовЗавершение", ЭтотОбъект);
		ПоказатьВопрос(ОписаниеОповещения, нСтр("ru='Текущие варианты ответов будут удалены. Продолжить?'", "ru"),РежимДиалогаВопрос.ДаНет, 60);
	Иначе
		СоздатьВариантОтвета(Объект.Ссылка, "Да", 1);
		СоздатьВариантОтвета(Объект.Ссылка, "Нет", 2);
		СоздатьВариантОтвета(Объект.Ссылка, "Неприменимо", 3);
		Элементы.ТаблицаВариантыОтветов.Обновить();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнениеВариантовОтветовЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
...
Процедура СоздатьВариантОтвета(тВладелец, тНаименование, тПорядок)
	Попытка
		нВопрос = Справочники.ВариантыОтветовАнкет.СоздатьЭлемент();
		нВопрос.Владелец = тВладелец;
		нВопрос.Наименование = тНаименование;
		нВопрос.РеквизитДопУпорядочивания = тПорядок;
		нВопрос.ТребуетОткрытогоОтвета = Ложь;
		нВопрос.Записать();
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не удалось создать вариант ответа: " + тНаименование + " по причине: " + ОписаниеОшибки();
		Сообщение.Сообщить(); 
	КонецПопытки;
КонецПроцедуры
// -- rarus
