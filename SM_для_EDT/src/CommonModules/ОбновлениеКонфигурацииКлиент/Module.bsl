...
Функция ПоддерживаетсяУстановкаОбновлений() Экспорт
	...
	Если ПараметрыРаботыКлиента.ИнформационнаяБазаФайловая И ПараметрыРаботыКлиента.ТребуетсяОбновлениеКонфигурацииУзлаРИБ Тогда
		
		ИнформацияОСоединенияхСтруктура = СоединенияИБВызовСервера.ИнформацияОСоединениях();
		Если ИнформацияОСоединенияхСтруктура.НаличиеСоединенияКонфигуратором Тогда
			Результат.ОписаниеОшибки = Результат.ОписаниеОшибки
			+ НСтр("ru = 'Для установки обновления требуется закрыть конфигуратор.'");
		КонецЕсли;
		
		Если ИнформацияОСоединенияхСтруктура.ЕстьАктивныеПользователи И ИнформацияОСоединенияхСтруктура.НаличиеАктивныхСоединений Тогда
			ИнформацияОБлокирующихСеансахСтруктура = СоединенияИБВызовСервера.ИнформацияОБлокирующихОбновлениеСеансах();
			Если ИнформацияОБлокирующихСеансахСтруктура.ИмеютсяБлокирующиеСеансы Тогда
				Результат.ОписаниеОшибки = Результат.ОписаниеОшибки + "
				|" + ИнформацияОБлокирующихСеансахСтруктура.ТекстСообщения;	
			КонецЕсли;	
		КонецЕсли;
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим, Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоОтложенноеОбновление Тогда 
		...
		ПараметрыСкрипта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("//nologo ""%1"" /p1:""%2"" /p2:""%3""",
			...
			СтрокаUnicode(vftОбщегоНазначения.ПолучитьПарольАдминистратора()),
			// -- rarus yukuzi 30.06.2021
