...
Процедура ДобавитьСтрокиДереваАнкеты(Выборка,СтрокаРодитель,УровеньРекурсии,Форма)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Форма.ИмяФормы = "Справочник.ШаблоныАнкет.Форма.ФормаЭлемента" Тогда
			...
			// ++ rarus ilshil 17.09.2021
			Если Не Выборка.ЭтоРаздел Тогда
				НоваяСтрока.IDВопроса  = Выборка.ЭлементарныйВопрос.IDВопроса;
			КонецЕсли;
			// -- rarus ilshil 17.09.2021		
		Иначе 
...
Процедура ДобавитьРеквизитыДляВопроса(СтрокаДерева,ДобавляемыеРеквизиты,Форма)
	...
	Иначе
		...
		Если (СтрокаДерева.ТипОтвета <> Перечисления.ТипыОтветовНаВопрос.НесколькоВариантовИз) И (СтрокаДерева.ТребуетсяКомментарий) Тогда
			// ++ rarus zalikh 2021-06-02 [29063]
			// было
			// ОписаниеТипаСтроки = Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(150));
			// стало
			ОписаниеТипаСтроки = Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(1000));
			// -- rarus zalikh 2021-06-02 [29063]
...
Процедура ДобавитьЭлементыВопрос(СтрокаТаблицы,ЭлементГруппа,Форма)
	...
	//++ rarus zalikh 2021-04-27 [27313]
	Если Форма.ИмяФормы = "Документ.Анкета.Форма.ФормаДокумента" Тогда
		ИмяКнопки = "Кнопка_" + СтрЗаменить(СтрокаТаблицы.КлючСтроки,"-","_");
		Элемент = Форма.Элементы.Добавить(ИмяКнопки, Тип("КнопкаФормы"), ЭлементГруппаВопросКомментарий);
		Элемент.Вид = ВидКнопкиФормы.Гиперссылка;
		Элемент.Заголовок = "Замечания";
		Элемент.ИмяКоманды = "ОткрытьСписокЗамечаний";
	КонецЕсли;
	//-- rarus zalikh 2021-04-27 [27313]
...
Процедура ПолучитьИнформациюПоВопросамАнкеты(Форма, ШаблонАнкеты, Раздел,ПолныйКодРаздела)
	...
	// ++ rarus ilshil 20.09.2021
	ИспользоватьIDВопроса = Форма.ШаблонАнкеты.ВыводитьIDВопросов;
	// -- rarus ilshil 20.09.2021
	...
	Пока ВыборкаВопросыПоРазделуАнкеты.Следующий() Цикл
		...
		НоваяСтрока.КлючСтроки = Новый УникальныйИдентификатор;	
		// ++ rarus ilshil 20.09.2021
		Если  ИспользоватьIDВопроса Тогда
			НоваяСтрока.ПолныйКод = НоваяСтрока.ЭлементарныйВопрос.IDВопроса;		
		Иначе			
