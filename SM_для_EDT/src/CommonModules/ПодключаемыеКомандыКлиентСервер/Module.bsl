		...
		Для Каждого Команда Из КраткиеСведенияОПодменю.КомандыСУсловиямиВидимости Цикл
			...
			Для Каждого Объект Из ВыбранныеОбъекты Цикл
				...
				Если ТипЗнч(Команда.УсловияВидимости) = Тип("Массив")
					...
					// В зависимости от состояния реквизита меняется выводимая команда
					//И Не УсловияВыполняются(Команда.УсловияВидимости, Объект) Тогда
					И (Не УсловияВыполняются(Команда.УсловияВидимости, Объект)
					ИЛИ Не ?(ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ВыгрузкаКритична"),
					       УсловияВыполняются(Команда.УсловияВидимости, Новый Структура("ВыгрузкаКритична", Форма.ВыгрузкаКритична)),
						   Истина)) Тогда
					// -- rarus makole 2020-10-29 [СТТ БСО.СМ.006]
			...
			Если ИзменятьВидимость 
				ИЛИ СтрНачинаетсяС(КомандаЭлемент.Имя, "ПодменюБСО") Тогда
			// -- rarus makole 2020-10-29 [СТТ БСО.СМ.006]
