...
Процедура ПриНачалеРаботыСистемы(Знач ОповещениеЗавершения = Неопределено, НепрерывноеВыполнение = Истина) Экспорт
	...
	рарусОбщегоНазначенияВызовСервера.СоздатьСценарииСинхронизации();
	
	ПриниматьДанныеПриЗапуске = ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекЗагрузить("ПриниматьДанныеПриЗапуске", "ПриниматьДанныеПриЗапуске");
	// ++ rarus Камаев П.В. 02.04.2021 Задача № 28365
	// Добавил условие НЕ полные права
	Если ПриниматьДанныеПриЗапуске = Истина И НЕ vftОбщегоНазначенияВызовСервера.ЭтоПолноправныйПользовательИБ() Тогда
	// -- rarus Камаев П.В. 02.04.2021
		Если НЕ рарусОбщегоНазначенияВызовСервера.ПолучитьКонфигурацияИзменена() Тогда
			//ДлительнаяОперация = рарусОбщегоНазначенияВызовСервера.ВыполнитьОбменСГлавнымУзлом(Истина);
			//
			...
			рарусОбщегоНазначенияВызовСервера.ОбменСГлавнымУзлом(Истина);
		КонецЕсли;
		Если рарусОбщегоНазначенияВызовСервера.ПолучитьКонфигурацияИзменена() Тогда
			рарусОбщегоНазначенияКлиент.ОбменСудноПринять();
		КонецЕсли;
	КонецЕсли;
...
Процедура ДействияПередНачаломРаботыСистемыОбработкаЗавершения(Неопределен, Контекст) Экспорт
	...
	// ++ rarus PleA 25.02.2021 
	Если Параметры.Перезапустить тогда
		
		Параметры.ДополнительныеПараметрыКоманднойСтроки = "/NАдминистратор /Pv0lgaflot! /CВыполнитьОбновлениеИЗавершитьРаботу;РегламентныеЗаданияОтключены;ПропуститьЗагрузкуПриоритетныхДанныхПередЗапуском";	
