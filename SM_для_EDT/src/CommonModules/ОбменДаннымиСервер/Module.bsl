...
Процедура ВыполнитьДействиеОбменаДляУзлаИнформационнойБазы(
	...
	Если СтруктураНастроекОбмена.Отказ Тогда
		...
		РегистрацияОбмена(Отказ, СтруктураНастроекОбмена);
...
Функция ТаблицаМонитораОбменаДанными(Знач ПланыОбмена, Знач ДополнительныеСвойстваПланаОбмена = "") Экспорт
	...
	|,  рарусВерсииКонфигурации.НомерВерсии КАК НомерВерсииНаСудне
	|,  рарусВерсииКонфигурации.НомерВерсииПлатформы КАК НомерВерсииПлатформыНаСудне
	// -- rarus PleA
	|ИЗ
	...
	|		ПО ОбщиеНастройкиУзловИнформационныхБаз.ПрефиксКорреспондента = рарусВерсииКонфигурации.УзелПрефикс
	// -- rarus PleA
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Если СтруктураНастроекОбмена.Отказ Тогда
				...
				// ++ rarus Камаев П.В. 04.06.2020 Задача № 22119
				РегистрацияОбмена(ОтказПоСтрокеСценария, СтруктураНастроекОбмена);
				// -- rarus Камаев П.В. 04.06.2020
				Продолжить;
			КонецЕсли;
...
Функция НастройкиОбменаДляУзлаИнформационнойБазы(
	...
	// ++ rarus makole 2021-03-09 
	// Сервис проверки IP отвечает не стабильно, пока отключить, рассмотреть альтернативные варианты получения IP
	
	//// ++ rarus makole 2020-12-10 [ПРОТОКОЛ Р-61 от 19.12.2020]
	//// 6.	Новое требование: Заблокировать возможность обмена РИБ (вариант – Полный) при спутниковом интернете во всех сценариях.
	//Если СтруктураНастроекОбмена.ЭтоОбменВРИБ И ОбменДаннымиСервер.ЭтоПодчиненныйУзелРИБ() Тогда
	//	ДанныеРегистрацииВСетиИнтернет = рарусСинхронизацияССудном.ПолучитьРегистрациюВСети(Истина);
	//	Если ДанныеРегистрацииВСетиИнтернет.ЭтоСпутник Тогда
	//		СтруктураНастроекОбмена.Отказ = Истина;
	//	   	СтруктураНастроекОбмена.РезультатВыполненияОбмена = СтрШаблон("Обмен данными в РИБ при использовании спутникового канала связи запрещён. Текущий адрес регистрации в сети: %1", ДанныеРегистрацииВСетиИнтернет.СтрокаРегистрации);
	//		СтруктураНастроекОбмена.СтрокаСообщенияОбОшибке = "Обмен прерван! Зона действия спутникового интернета!";
	//	КонецЕсли;
	//КонецЕсли;
	//// -- rarus makole 2020-12-10 [ПРОТОКОЛ Р-61 от 19.12.2020]
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		...
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
		// ++ rarus Камаев П.В. 04.06.2020 Задача № 22119
		СтруктураНастроекОбмена.Вставить("СтрокаСообщенияОбОшибке", СтрокаСообщенияОбОшибке);
		// -- rarus Камаев П.В. 04.06.2020
...
// ++ rarus Камаев П.В. 04.06.2020 Задача № 22119
Процедура РегистрацияОбмена(Отказ, СтруктураНастроекОбмена, Описание = "")
	
	Если vftОбщегоНазначения.ЭтоГлавныйУзел() Тогда
		Возврат;
	КонецЕсли;
	
	Источник = СтруктураНастроекОбмена.ТекущийУзелПланаОбмена;
	Приемник = СтруктураНастроекОбмена.УзелИнформационнойБазы;
	...
	Если Отказ Тогда
		...
		Если НЕ ЗначениеЗаполнено(Описание) Тогда
			Описание = СтруктураНастроекОбмена.СтрокаСообщенияОбОшибке;
			Описание = СтрПолучитьСтроку(Описание, 1);
			Если СтрНайти(Описание, "Номер сообщения меньше или равен номеру ранее принятого сообщения") <> 0 Тогда
				Описание = "Данные были загружены ранее. Нет новых данных для загрузки.";	
	...
	Иначе
		Если СтруктураНастроекОбмена.ДействиеПриОбмене = Перечисления.ДействияПриОбмене.ВыгрузкаДанных Тогда
			...
			Если НЕ ЗначениеЗаполнено(Описание) Тогда
				Описание = "Выгрузка данных завершена успешно.";
			КонецЕсли;
		Иначе
			РезультатОбмена = Перечисления.рарусРезультатыОбмена.ЗагрузкаОК;
			Если НЕ ЗначениеЗаполнено(Описание) Тогда
				Описание = "Загрузка данных завершена успешно.";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Запись = РегистрыСведений.рарусРегистрацияОбмена.СоздатьМенеджерЗаписи();
	Запись.Активность		= Истина;
	Запись.Судно			= Судно;
	Запись.Дата				= ТекущаяДата();
	Запись.РезультатОбмена	= РезультатОбмена;
	Запись.Описание			= Описание;
	Запись.Записать();
	
	//очистка старых сообщений, < 90 дней
	ОчисткаРегистраРегистрацииОбмена();
	
КонецПроцедуры

Процедура ОчисткаРегистраРегистрацииОбмена() Экспорт // rarus makole 13092021 Добавил Экспорт
	
	//Состояние("Очистка регистра регистрации обмена...");
	
	// Регистр "РегистрацияОбмена", удаляем записи старше 90 дней
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТекДата",ТекущаяДата());
	Запрос.Текст = "ВЫБРАТЬ *
	|ИЗ
	|	РегистрСведений.рарусРегистрацияОбмена КАК Рег
	|ГДЕ
	|	РАЗНОСТЬДАТ(Рег.Дата, &ТекДата, ДЕНЬ) <= 90";
	
	НаборЗаписей = РегистрыСведений.рарусРегистрацияОбмена.СоздатьНаборЗаписей();
