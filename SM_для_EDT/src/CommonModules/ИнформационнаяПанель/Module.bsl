...
Функция НастройкаПоУмолчанию() Экспорт
	...
	//ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаЗаказыНаВнутреннееПотребление());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаОстаткиСудовогоСклада());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаПлановыеВнеплановыеРемонты());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаЗапланированныеМероприятияСУБ());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаПриближениеСрокаСудовых());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаНеустраненныеЗамечанияСУБ());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаПросроченныеРемонты());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаОткрытиеАРМ());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаОбменДаннымиБМ());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаАнализСУБКапитанами());
	ВыбранныеБлоки.Добавить(БлокиИнформационнойПанелиКлиентСервер.ИмяБлокаОтслеживаниеПоказанийНаработки());
	...
	НастройкаБлоков.Вставить("КоличествоСтрок", КоличествоСтрок());
	НастройкаБлоков.Вставить("КоличествоКолонок", КоличествоКолонок());
...
//  КоллекцияБлоков - ДанныеФормыКоллекция, ТаблицаЗначений
//
Процедура ПрименитьНастройки(КоллекцияБлоков, КоличествоСтрок, КоличествоКолонок) Экспорт
	...
	НастройкаБлоков.Свойство("КоличествоСтрок", КоличествоСтрок);
	НастройкаБлоков.Свойство("КоличествоКолонок", КоличествоКолонок);
...
Процедура ВставитьБлокВНастройки(Настройки, ИмяБлока, ВставитьПослеБлока) Экспорт
	...
	МаксимальноеКоличествоБлоков = (Настройки.КоличествоСтрок * Настройки.КоличествоКолонок);
