...
Функция ЗаменитьСсылки(Знач ПарыЗамен, Знач ПараметрыЗамены = Неопределено) Экспорт
	...
	Попытка
		...
		// ++ rarus iljyvs 30.06.2020
		ДобавитьВОчередьОбработкиДублей = Ложь;
		Если ПараметрыЗамены.Свойство("ДобавитьВОчередьОбработкиДублей", ДобавитьВОчередьОбработкиДублей)
			И ДобавитьВОчередьОбработкиДублей Тогда
			
			ОчередьОбработкиДублейДобавить(ПараметрыВыполнения);
...
// ++ rarus iljyvs 30.06.2020
Процедура ОчередьОбработкиДублейДобавить(ПараметрыВыполнения)
	...
	КодыУзловМассив = ОчередьОбработкиДублейКодыУзловМассивПолучить();
	Для Каждого ДубльОригинал Из ПараметрыВыполнения.УспешныеЗамены Цикл
		
		Для Каждого КодУзла Из КодыУзловМассив Цикл
			
			ЗаписьМенеджер = РегистрыСведений.рарусОчередьОбработкиДублей.СоздатьМенеджерЗаписи();
			
			ЗаписьМенеджер.КодУзла 		= КодУзла;
			ЗаписьМенеджер.Дубль 		= ДубльОригинал.Ключ;
			ЗаписьМенеджер.Оригинал 	= ДубльОригинал.Значение;
			ЗаписьМенеджер.Обработано 	= Ложь;
			ЗаписьМенеджер.ОшибкаТекст 	= "";
			
			Если ЗначениеЗаполнено(ЗаписьМенеджер.Дубль)
				И ЗначениеЗаполнено(ЗаписьМенеджер.Оригинал) Тогда
...
Функция ОчередьОбработкиДублейКодыУзловМассивПолучить()
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Результат.Добавить(Выборка.Код);	
