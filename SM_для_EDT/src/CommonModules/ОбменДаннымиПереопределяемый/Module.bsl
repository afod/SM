...
//++rarus markim	
	...
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.впОбменТОиР20СМ31);
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.рарусОбменУправлениеПредприятиемСудовойМодуль);
//--rarus markim	
КонецПроцедуры
...
//++rarus markim	
	 Если    ТипЗнч(Получатель) = Тип("ПланОбменаСсылка.Полный")
		 Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Получатель, "ИспользоватьОтборПоСудам, ДатаНачалаВыгрузкиДокументов, Суда");
		
		Суда = ?(ЗначенияРеквизитов.ИспользоватьОтборПоСудам, ЗначенияРеквизитов.Суда.Выгрузить().ВыгрузитьКолонку("Судно"), Неопределено);
		
		ОбменДаннымиСервер.ЗарегистрироватьДанныеПоДатеНачалаВыгрузкиИОрганизациям(Получатель, ЗначенияРеквизитов.ДатаНачалаВыгрузкиДокументов, Суда);
...
Процедура ПриКоллизииИзмененийДанных(Знач ЭлементДанных, ПолучениеЭлемента, Знач Отправитель, Знач ПолучениеОтГлавного) Экспорт
	...
	//Возврат;
	Если ТипЗнч(ЭлементДанных) = Тип("ДокументОбъект.vftРейс")
		И ТипЗнч(Отправитель) = Тип("ПланОбменаСсылка.Полный") Тогда
