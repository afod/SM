...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ++ rarus kloa 09.12.2019  
	//РольДоступнаПолныеПрава                     = Пользователи.ЭтоПолноправныйПользователь();
	РольДоступнаПолныеПрава                     = Пользователи.ЭтоПолноправныйПользователь() ИЛИ ОбновлениеКонфигурации.ЕстьПраваНаУстановкуОбновления();
	// -- rarus kloa 09.12.2019 
...
Процедура ЗавершениеДлительнойОперации()
	...
	//Для автообновления
	ОповеститьОбИзменении(Тип("РегистрСведенийКлючЗаписи.рарусРегистрацияОбмена"));
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	ИспользоватьПрием		= Истина;
	ИспользоватьОтправку	= Истина;
	ИспользоватьЗавершение	= Истина;
	
	Если Параметры.Свойство("ИспользоватьПрием") Тогда 
		ИспользоватьПрием = Параметры.ИспользоватьПрием;
	КонецЕсли;
	Если Параметры.Свойство("ИспользоватьОтправку") Тогда 
		ИспользоватьОтправку = Параметры.ИспользоватьОтправку;
	КонецЕсли;
	Если Параметры.Свойство("ИспользоватьЗавершение") Тогда 
		ИспользоватьЗавершение = Параметры.ИспользоватьЗавершение;
	КонецЕсли;
	// Камаев П.В. конец
	...
	Иначе
		...
		Иначе
			...
			Если ИспользоватьПрием Тогда
				ТаблицаПереходовНоваяСтрока(ИмяСтраницыСинхронизацияЗагрузка, "ЗагрузкаДанных_ПриОткрытии", Истина, "ЗагрузкаДанных_ОбработкаДлительнойОперации");
			КонецЕсли;
			// Отправка
			//ТаблицаПереходовНоваяСтрока(ИмяСтраницыСинхронизацияВыгрузка, "ВыгрузкаДанных_ПриОткрытии", Истина, "ВыгрузкаДанных_ОбработкаДлительнойОперации");
			//ТаблицаПереходовНоваяСтрока(ИмяСтраницыСинхронизацияВыгрузка, , Истина, "ВыгрузкаДанных_ОбработкаДлительнойОперации_Окончание");
			Если ИспользоватьОтправку Тогда
				ТаблицаПереходовНоваяСтрока(ИмяСтраницыСинхронизацияВыгрузка, "ВыгрузкаДанных_ПриОткрытии", Истина, "ВыгрузкаДанных_ОбработкаДлительнойОперации");
				ТаблицаПереходовНоваяСтрока(ИмяСтраницыСинхронизацияВыгрузка, , Истина, "ВыгрузкаДанных_ОбработкаДлительнойОперации_Окончание");
			КонецЕсли;
			// Камаев П.В. конец
		...
		Если ИспользоватьЗавершение Тогда
			ТаблицаПереходовНоваяСтрока("ЗавершениеОбмена", "ЗавершениеОбмена_ПриОткрытии", Истина, "ЗавершениеОбмена_ОбработкаДлительнойОперации");
		КонецЕсли;
		// Камаев П.В. конец
	КонецЕсли;
