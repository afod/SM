...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ++ rarus makole 2020-12-02 [БСО.СМ.006]
	Если ОбменДаннымиСервер.ЭтоПодчиненныйУзелРИБ() Тогда
		Элементы.рарусВыгружатьНаСудно.Видимость = Ложь;
	КонецЕсли;
	// -- rarus makole 2020-12-02 [БСО.СМ.006]
	
	// ++ rarus PleA 18.01.2021 [26397]
	// РаботаСФайламиСтрок
	Параметры.Свойство("СтрокаТаблицы", СтрокаТаблицы);	
	Параметры.Свойство("ИмяТаблицыВладельца", ИмяТаблицыВладельца);	
	Параметры.Свойство("ИмяТаблицыХраненияВладельца", ИмяТаблицыХраненияВладельца);
	ЗаполнитьФайлыОтбора(Параметры.ФайлыОтбор);	
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	// ++ rarus makole 2020-12-01 [БСО.СМ.006]
	Если Поле.Имя = "рарусВыгружатьНаСудно" Тогда
		Возврат;
	КонецЕсли;
	// -- rarus makole 2020-12-01 [БСО.СМ.006]
...
Процедура СписокПередНачаломИзменения(Элемент, Отказ)
	...
	Отказ = Истина;
	// ++ rarus makole 2020-12-01 [БСО.006]
	Если Элемент.ТекущийЭлемент.Имя = "рарусВыгружатьНаСудно" Тогда
		ИзменитьКритичностьВыгрузки(Элементы.Список.ТекущиеДанные.Ссылка);
		Элементы.Список.Обновить();
		Возврат;
	КонецЕсли;
	// -- rarus makole 2020-12-01 [БСО.006]
...
Процедура СписокПриИзменении(Элемент)
	...
	// ++ rarus PleA 19.01.2021 [номерзадачи]
	Оповестить("Запись_Файл", 
			Новый Структура("Событие, ИмяТаблицы, ВыбраннаяСтрока", "ДанныеФайлаИзменены", ИмяТаблицыВладельца, СтрокаТаблицы), 
...
Процедура ИзменитьКритичностьВыгрузки(Источник)
	
	КритичностьУстановлена = рарусСинхронизацияССудном.КритичностьУстановлена(Источник);
...
КонецПроцедуры // -- rarus makole 2020-12-01 [БСО.СМ.006]
...
Процедура ДобавитьФайл()
	...
	Иначе
		...
		// РаботаСФайламиСтрок
		РаботаСФайламиКлиент.ДобавитьФайлы(Параметры.ВладелецФайла, УникальныйИдентификатор, , ГруппаФайлов, ОписаниеОповещения);
		// Конец РаботаСФайламиСтрок
		// -- rarus PleA
...
Процедура НастроитьДинамическийСписок(ИмяСправочникаХранилищаФайлов, Знач ЕстьРеквизитСлужебный = Ложь)
	...
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК рарусВыгружатьНаСудно
	// -- rarus makole 2020-12-01 [БСО.СМ.006]
	|ИЗ
	...
	|		ПО Файлы.Ссылка = рарусКритичныеДанныеДляВыгрузкиНаСудно.Объект
	// -- rarus makole 2020-12-01 [БСО.СМ.006]
	|ГДЕ
...
// РаботаСФайламиСтрок
Процедура ЗаполнитьФайлыОтбора(МассивФайлов)
	
	Для Каждого Эл Из МассивФайлов Цикл 
		
		ФайлыОтбор.Добавить(Эл);
		
	КонецЦикла;
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Ссылка", ФайлыОтбор, ВидСравненияКомпоновкиДанных.ВСписке);
...
Процедура ДобавитьФайлЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат  <> Неопределено тогда
		...
		ЗаполнитьФайлыОтбора(Результат);	
	
		ДополнительныеПараметры = рарусРаботаСФайламиСтрокКлиент.ПараметрыДобавленияФайла(
			СтрокаТаблицы, 
			ВладелецФормы, 
			ИмяТаблицыВладельца, 
			ИмяТаблицыХраненияВладельца
			);
	
		ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьФайлПоСтрокеЗавершение", 
			рарусРаботаСФайламиСтрокКлиент, 
