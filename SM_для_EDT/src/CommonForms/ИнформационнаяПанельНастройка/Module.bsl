...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МаксимальноеКоличествоСтрок		= ИнформационнаяПанель.КоличествоСтрок();
	МаксимальноеКоличествоКолонок	= ИнформационнаяПанель.КоличествоКолонок();
...
Процедура КоличествоСтрокПриИзменении(Элемент)
	...
	Если КоличествоСтрок > МаксимальноеКоличествоСтрок Тогда
		КоличествоСтрок = МаксимальноеКоличествоСтрок
	ИначеЕсли КоличествоСтрок = 0 Тогда
		КоличествоСтрок = 1
	КонецЕсли;
	МаксимальноеКоличествоБлоков = КоличествоСтрок * КоличествоКолонок;
	ПроверитьУдалитьЛишниеБлоки();
	
КонецПроцедуры

&НаКлиенте
Процедура КоличествоКолонокПриИзменении(Элемент)
	
	Если КоличествоКолонок > МаксимальноеКоличествоКолонок Тогда
		КоличествоКолонок = МаксимальноеКоличествоКолонок
...
&НаКлиенте
Процедура ПроверитьУдалитьЛишниеБлоки()
	
	КоличествоВыбранныхБлоков = ВыбранныеБлоки.Количество();
	Если КоличествоВыбранныхБлоков > МаксимальноеКоличествоБлоков Тогда
		Разность = КоличествоВыбранныхБлоков - МаксимальноеКоличествоБлоков;
		Удалено = 0;
		Пока Удалено < Разность Цикл
			УдалитьБлок(ВыбранныеБлоки[КоличествоВыбранныхБлоков - 1 - Удалено]);
			Удалено = Удалено + 1;
		КонецЦикла;
	Иначе
		УправлениеФормой(ЭтотОбъект);
